    ...
    <div class="container">
        ...
        <div class="row">
            <div class="card text-bg-secondary col-sm-6 mb-3 mb-sm-0 text-center">
                <div class="card-header">
                    <blockquote class="blockquote mb-0">
                        <h4>Actualización de Entrada y Salida <br> Administrativos</h4>
                    </blockquote>
                </div>
                <div class="card-body">

                    <form action="administrative/loadBiometricTime.php" method="post" enctype="multipart/form-data" id="formulario" name="formulario">
                        <input hidden type="text" value="1" name="send">
                        <div class="modal-body">
                            <div class="input-group mb-3">
                                <!--input type="file" name="archivo_csv" class="form-control" id="inputGroupFile01" onchange="checkFile(this, ['.csv']);"-->
                                <input type="file" name="archivo_csv" class="form-control" id="inputGroupFile01">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="submit" id="buttonSend" class="btn btn-primary btn-formulario" disabled>Enviar</button>
                        </div>
                    </form>
                </div>
            </div>
...
document.getElementById("inputGroupFile01").addEventListener("change", function() {
    ...
    if (file) {
        ...
        if (ext == 'csv') {
            console.log("Se habilita botón de enviar");
            var boton = document.getElementById("buttonSend");
            console.log(boton);
            boton.disabled = false;
        } else {
            console.log("Incorrect File Type: No se habilita el botón");
            var boton = document.getElementById("buttonSend");
            console.log(boton);
            boton.disabled = true;
        }
