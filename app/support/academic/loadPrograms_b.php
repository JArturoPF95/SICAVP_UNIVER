...
$sqlDelete = "DELETE FROM academic_tolerance";
if ($mysqli->query($sqlDelete)) {
    ...
    if (isset($_FILES['archivo_xlsx'])) {
        ...
        if ($archivo_xlsx['error'] === UPLOAD_ERR_OK) {
            ...
            $encabezados[0] = 'CODE_DEGPRO';
            $encabezados[1] = 'CODE_DEGREE';
            $encabezados[2] = 'DEGREE';
            $encabezados[3] = 'CODE_PROGRAM';
            $encabezados[4] = 'PROGRAM';
            $encabezados[5] = 'DESCRIPTION';
            ...
            try{
                ...
                while (($fila = fgetcsv($csvFile)) !== false) {
                    ...
                    $insert = "INSERT IGNORE INTO academic_tolerance ($columnas) VALUES ('$valores');";
                    if ($mysqli->query($insert)) {
                        $message = 'Layout de Nivel - Programa Docentes Convertido y Cargado correctamente.';
                        $icon = 'success';
                    } else {
                        $message = 'Error cargando Nivel - Programa Docentes';
                        $icon = 'error';
                    }
                }
...
<body>
    ...
    <script type="text/javascript">
        swal({
            title: "Carga de Nivel - Programa Docentes",
