        ...
        <form class="needs-validation" novalidate method="post" action="administrative/userCreate.php">
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select" id="sesion" name="sesion" style="font-size: 13px;" required>
                        <?php
                        $sqlSesion = "SELECT PK, CITY, NOM_SESION FROM code_sesion ORDER BY CITY, NOM_SESION ASC";
                        $resultSesion = $mysqli->query($sqlSesion);
                        if ($resultSesion->num_rows > 0) {
                            while ($rowSesion = $resultSesion->fetch_assoc()) {
                        ?>
                                <option value="<?php echo $rowSesion['PK'] ?>"><?php echo $rowSesion['CITY'] . ' - ' . $rowSesion['NOM_SESION'] ?></option>
                        <?php
                            }
                        }
                        ?>
                ...
                <div class="col-sm-6">
                    ...
                    <select class="form-select" id="access" name="access" style="font-size: 13px;" required>
                        <?php
