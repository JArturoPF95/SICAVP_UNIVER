...
$sqlDelete = "DELETE FROM groups_daytrip WHERE FLEX_SCHEDULE = '0'";
if ($mysqli->query($sqlDelete)) {
    ...
    if (isset($_FILES['archivo_xlsx'])) {
        ...
        if ($archivo_xlsx['error'] === UPLOAD_ERR_OK) {
            ...
            $encabezados[0] = 'CODE_GROUP';
            $encabezados[1] = 'GROUP_NAME';
            $encabezados[2] = 'CODE_DAYTRIP';
            $encabezados[3] = 'DAYTRIP';
            $encabezados[4] = 'ROLE_ORDER';
            ...
            try {
                ...
                while (($fila = fgetcsv($csvFile)) !== false) {
                    ...
                    $insert = "INSERT IGNORE INTO groups_daytrip ($columnas) VALUES ('$valores');";
                    if ($mysqli->query($insert)) {
                        $message = 'Layout de Grupos - Jornada Convertido y Cargado correctamente.';
                        $icon = 'success';
                    } else {
                        $message = 'Error cargando Grupos - Jornada';
                        $icon = 'error';
                    }
                }
...
<head>
    ...
    <title>Envío de Grupos - Jornada</title>
