...
$sqlDelete = "DELETE FROM code_jobs";
if ($mysqli->query($sqlDelete)) {
    ...
    if (isset($_FILES['archivo_xlsx'])) {
        ...
        if ($archivo_xlsx['error'] === UPLOAD_ERR_OK) {
            ...
            $encabezados[0] = 'CODE_JOB';
            $encabezados[1] = 'JOB_NAME';
            ...
            try{
                ...
                while (($fila = fgetcsv($csvFile)) !== false) {
                    ...
                    $insert = "INSERT IGNORE INTO code_jobs ($columnas) VALUES ('$valores');";
                    if ($mysqli->query($insert)) {
                        $message = 'Layout de Puestos Convertido y Cargado correctamente.';
                        $icon = 'success';
                    } else {
...
<body>
    ...
    <script type="text/javascript">
        swal({
            title: "Carga de Puestos",
            text: "<?php echo $message; ?>",
            icon: "<?php echo $icon ?>",
            button: "Volver",
        }).then(function() {
            window.location = "../jobs.php?id=<?php echo $user_active ?>";
        });
    </script>
