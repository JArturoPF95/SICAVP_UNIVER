...
$sqlDelete = "DELETE FROM code_position";
if ($mysqli->query($sqlDelete)) {
    ...
    if (isset($_FILES['archivo_xlsx'])) {
        ...
        if ($archivo_xlsx['error'] === UPLOAD_ERR_OK) {
            ...
            //Asignamos nombre al encabezado: Modificar cuando llegue el nuevo Layout y también base
            $encabezados[0] = 'CODE_POSITION';
            $encabezados[1] = 'POSITION_DESCRIPTION';
            $encabezados[2] = 'CODE_JOB';
            $encabezados[3] = 'CODE_NOM_SESSION';
            $encabezados[4] = 'CODE_DEPARTMENT';
            $encabezados[5] = 'CODE_AREA';
            $encabezados[6] = 'BOSS_POSITION';
            ...
            try{
                ...
                while (($fila = fgetcsv($csvFile)) !== false) {
                    ...
                    $insert = "INSERT IGNORE INTO code_position ($columnas) VALUES ('$valores');";
                    if ($mysqli->query($insert)) {
                        $message = 'Layout de Posiciones Convertido y Cargado correctamente.';
                        $icon = 'success';
                    } else {
...
<head>
    ...
    <title>Envío de Posiciones</title>
...
<body">
    ...
    <script type="text/javascript">
        swal({
            title: "Carga de Posiciones",
            text: "<?php echo $message; ?>",
            icon: "<?php echo $icon ?>",
            button: "Volver",
        }).then(function() {
            window.location = "../positions.php?id=<?php echo $user_active ?>";
        });
    </script>
