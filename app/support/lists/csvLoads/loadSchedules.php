...
$sqlDelete = "DELETE FROM code_schedule WHERE FLEX_SCHEDULE != '1'";
if ($mysqli->query($sqlDelete)) {
    ...
    if (isset($_FILES['archivo_xlsx'])) {
        ...
        if ($archivo_xlsx['error'] === UPLOAD_ERR_OK) {
            ...
            $encabezados[0] = 'CODE_NOM';
            $encabezados[1] = 'DAYTRIP';
            $encabezados[2] = 'SCHEDULE';
            ...
            try {
                ...
                while (($fila = fgetcsv($csvFile)) !== false) {
                    ...
                    $insert = "INSERT IGNORE INTO code_schedule ($columnas) VALUES ('$valores');";
                    if ($mysqli->query($insert)) {
                            $message = 'Layout de Jornadas Convertido y Cargado correctamente.';
                            $icon = 'success';
                    } else {
                        $message = 'Error cargando Jornadas';
                        $icon = 'error';
                    }
                }
