...
$sqlDelete = "DELETE FROM code_incidence WHERE CODE_TINC < 7";
if ($mysqli->query($sqlDelete)) {
    ...
    if (isset($_FILES['archivo_xlsx'])) {
        ...
        if ($archivo_xlsx['error'] === UPLOAD_ERR_OK) {
            ...
            $encabezados[0] = 'CODE_TINC';
            $encabezados[1] = 'DESCRIP_TINC';
            ...
            try{
                ...
                while (($fila = fgetcsv($csvFile)) !== false) {
                    ...
                    $insert = "INSERT IGNORE INTO code_incidence ($columnas) VALUES ('$valores');";
                    if ($mysqli->query($insert)) {
                        $message = 'Layout de Tipos de Faltas Convertido y Cargado correctamente.';
                        $icon = 'success';
                    } else {
                        $message = 'Error cargando Tipos de Faltas';
                        $icon = 'error';
                    }
                }
...
<head>
    ...
    <title>Envío de Tipos de Faltas</title>
...
<body">
    ...
    <script type="text/javascript">
        swal({
            title: "Carga de Tipos de Faltas",
            text: "<?php echo $message; ?>",
            icon: "<?php echo $icon ?>",
            button: "Volver",
        }).then(function() {
            window.location = "../incidences.php?id=<?php echo $user_active ?>";
        });
    </script>
