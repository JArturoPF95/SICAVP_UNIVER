...
$sqlDelete = "DELETE FROM code_sesion";
if ($mysqli->query($sqlDelete)) {
    ...
    if (isset($_FILES['archivo_xlsx'])) {
        ...
        if ($archivo_xlsx['error'] === UPLOAD_ERR_OK) {
            ...
            $encabezados[0] = 'CODE_SESION_NOM';
            $encabezados[1] = 'NOM_SESION';
            $encabezados[2] = 'CODE_COUNTRY';
            $encabezados[3] = 'COUNTRY';
            $encabezados[4] = 'CODE_CITY';
            $encabezados[5] = 'CITY';
            ...
            try {
                ...
                while (($fila = fgetcsv($csvFile)) !== false) {
                    ...
                    $insert = "INSERT IGNORE INTO code_sesion ($columnas) VALUES ('$valores');";
                    if ($mysqli->query($insert)) {
                        $message = 'Layout de Campus Convertido y Cargado correctamente.';
                        $icon = 'success';
                    } else {
                        $message = 'Error cargando Campus';
                        $icon = 'error';
                    }
                }
...
<body>
    ...
    <script type="text/javascript">
        swal({
            title: "Carga de Campus",
            text: "<?php echo $message; ?>",
            icon: "<?php echo $icon ?>",
            button: "Volver",
        }).then(function() {
            window.location = "../sesions.php?id=<?php echo $user_active ?>";
        });
    </script>
