...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($flag == '0') {
        $sqlVoid = "UPDATE vacation_anticipated SET COMMENTS = '$comments', ACTIVE_FLAG = '0', REQUEST_FLAG = 0, MODIFIED_BY = '$user_active', MODIFIED_DATE = NOW() WHERE ID_NOM = '$idNom'";
        ...
        $sqlActivate = "UPDATE vacation_anticipated SET DAYS = '$days', COMMENTS = '$comments', ACTIVE_FLAG = '1', REQUEST_FLAG = 0, MODIFIED_BY = '$user_active', MODIFIED_DATE = NOW() WHERE ID_NOM = '$idNom'";
...
<body>
    <div class="container">
        ...
        <div class="row my-3">
            <?php
            $sqlGetAnticipated = "SELECT EMP.ID_NOM ID, CONCAT(EMP.LAST_NAME,' ',EMP.LAST_NAME_PREFIX,' ',EMP.NAME) NAME, EMP.ANTIQUITY, EMP.SUPERVISOR_ID, VAA.ACTIVE_FLAG, VAA.DAYS, VAA.COMMENTS, VAA.REQUEST_FLAG
    FROM employed EMP
                ...
                <table class="table table-primary table-hover table-bordered table-sm" style="font-size: 13px;">
                    <thead class="text-white text-center">
                        <tr class="text-center">
                            <th scope="col" >Periodo <br> Anticipado</th>
                    ...
                    while ($rowAnticipated = $resultAnticipated->fetch_assoc()) {
                        ...
                        $requestFlag = $rowAnticipated['REQUEST_FLAG'];
                    ?>
                        ...
                        <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
                            <tbody class="table-light">
                                <tr class="text-center">
                                    ...
                                    <td ><input type="number" name="days" value="<?php echo $days ?>" required></td>
                                    <td ><textarea name="comments" maxlength="250" rows="1" cols="50" required><?php $comments = ($activeFlag == '0') ? 'Anulado ' : $comments;
                                                                                                                                    echo $comments; ?></textarea></td>
                                    <td > <?php
                                                            if ($activeFlag == '1') {
                                                                if ($requestFlag == '1') {
                                                                    echo 'Vacaciones ya solicitadas';
                                                                } else {
