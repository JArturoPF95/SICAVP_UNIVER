...
<body>
    ...
    ?>
        ...
        <div class="d-flex gap-2 py-2">
            ...
            ?>
                <table id="myTable" class="table table-primary table-hover table-bordered table-sm" style="font-size: 13px;">
                                ...
                                <td>
                                    <div class="btn-group dropend">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li class="dropdown-item"><a class="dropdown-item" href="academic/user_reset.php?id=<?php echo $rowUsers['SICAVP_USER'] ?>"><i class="bi bi-person-fill-gear"></i> &nbsp; Reestablecer Contraseña</a></li>
                                            <?php
                                            if ($rowUsers['SEPARATION_FLAG'] == '0') {
                                            ?>
                                                <li class="dropdown-item"><a class="dropdown-item" href="academic/user_lock.php?id=<?php echo $rowUsers['SICAVP_USER'] ?>"><i class="bi bi-person-fill-lock"></i> &nbsp; Bloquear Docente</a></li>
                                            <?php
                                            } elseif ($rowUsers['SEPARATION_FLAG'] == '1') {
                                            ?>

                                                <li class="dropdown-item"><a class="dropdown-item" href="academic/user_unlock.php?id=<?php echo $rowUsers['SICAVP_USER'] ?>"><i class="bi bi-person-fill-up"></i> &nbsp; Desbloquear Docente</a></li>
                                            <?php
                                            }
                                            ?>
...
?>
    ...
    <div class="container text-center mb-2">
        <div class="row">
            ...
            <div class="col">
                <!--Carga Masiva de Layout-->
                <a type="button" href="academic/loadPrograms.php" class="btn btn-secondary">
                    Actualiza Programas PwC &nbsp; <i class="bi bi-arrow-clockwise"></i>
                </a>
            </div>
        </div>
    </div>
    <div class="col-md-11 col-lg-11 d-flex justify-content-center py-2 px-5">
        ...
        ?>
            <table id="myTable" class="table table-primary table-hover table-bordered table-sm" style="font-size: 13px;">
                <thead class="text-center">
                    <tr>
                        <th scope="col" class="text-white">Programa</th>
                        <th scope="col" class="text-white">Minutos de Inicio <br> Anticipado de Clase</th>
                        <th scope="col" class="text-white">Minutos de Tolerancia</th>
                        <th scope="col" class="text-white">Minutos máximo <br> de Ingreso</th>
                        <th scope="col" class="text-white">Minutos permitidos para Cierre <br> Anticipado de Clase</th>
                        <td scope="col"></td>
                    </tr>
                </thead>
                <tbody class="table-light text-center">
                    ...
                    ?>
                        <form action="academic/tolerance.php" method="post">
                            <tr>
                                <td hidden><input type="text" name="send" value="2"></td>
                                <td hidden><input type="text" name="prog" value="<?php echo $rowTolerance['CODE_PROGRAM'] ?>"></td>
                                <td hidden><input type="text" name="progDesc" value="<?php echo $rowTolerance['DESCRIPTION'] ?>"></td>
                                ...
                                <td><button type="submit" class="btn btn-secondary btn-sm" value="<?php echo $rowTolerance['CODE_PROGRAM'] ?>">Actualizar</button></td>
                            </tr>
                        </form>
                    <?php
                    }
...
?>
    ...
    <div class="d-flex gap-2 py-2">
        <?php
        $sqlSesionPwC = "SELECT DISTINCT CSE.PK, CSE.CITY, CSE.NOM_SESION, CSA.* FROM code_sesion_academic CSA LEFT OUTER JOIN code_sesion CSE ON CSE.PK = CSA.CODE_NOM WHERE CODE_VALUE_KEY != 'SELVAL'";
        ...
        ?>
            <table id="myTable" class="table table-primary table-hover table-bordered table-sm" style="font-size: 13px;">
                ...
                while ($rowSesionPwC = $resultSesionPwC->fetch_assoc()) {
                    $pkSesion = $rowSesionPwC['PK'];
                    $codeValuePwC = $rowSesionPwC['CODE_VALUE_KEY'];
                    $longDescPwC = $rowSesionPwC['LONG_DESC'];
                    $cityNom = $rowSesionPwC['CITY'];
                    $nomSesion = $rowSesionPwC['NOM_SESION'];
                ...
                ?>
                    <tbody class="text-center">
                        <form action="academic/updateNomSesion.php" method="post">
                            <tr class="<?php echo $bg ?>">
                                <td hidden><input type="text" name="codePwC" value="<?php echo $codeValuePwC ?>"></td>
                                <td hidden><input type="text" name="send" value="3"></td>
                                <td hidden><?php echo $pkSesion ?></td>    
                                <td><?php echo $longDescPwC ?></td>
                                <?php
                                if ($pkSesion === '') {
                                    $sqlGetNomSesion = "SELECT CSA.CODE_VALUE_KEY, CSA.LONG_DESC, CSN.CODE_CITY, CSN.CITY, CSN.NOM_SESION, CSN.PK FROM code_sesion CSN LEFT OUTER JOIN code_sesion_academic CSA  ON CSN.PK = CSA.CODE_NOM";
                                    $resultGetNomSesion = $mysqli->query($sqlGetNomSesion);
                                    ?>                                
                                <td>
                                    <!--?php echo $sqlGetNomSesion;?-->  
                                    <select class="form-select" name="idNom2001" style="font-size: 13px;" required>
                                                    <option selected disabled>Asignar Campus de Nom2001</option>
                                                    <?php
                                                    while ($rowNomSesion = $resultGetNomSesion->fetch_assoc()) {
                                                    ?>
                                                        <option value="<?php echo $rowNomSesion['PK'] ?>"><?php echo $rowNomSesion['CITY'] . ' - ' . $rowNomSesion['NOM_SESION'] ?></option>
                                                    <?php
                                                    }
                                                    ?>
                                                </select>
                                    ...
                                    $sqlGetNomSesion = "SELECT CSA.CODE_VALUE_KEY, CSA.LONG_DESC, CSN.CODE_CITY, CSN.CITY, CSN.NOM_SESION, CSN.PK FROM code_sesion CSN LEFT OUTER JOIN code_sesion_academic CSA  ON CSN.PK = CSA.CODE_NOM WHERE CSN.PK <> '$pkSesion'";
                                    $resultGetNomSesion = $mysqli->query($sqlGetNomSesion);
                                    ?>                               
                                <td>
                                    <!--?php echo $sqlGetNomSesion;?-->
                                    <select class="form-select" name="idNom2001" style="font-size: 13px;" required>
                                    <option selected value="<?php echo $pkSesion ?>"><?php echo $cityNom . ' - ' . $nomSesion ?></option>
                                    <?php
                                    while ($rowNomSesion = $resultGetNomSesion->fetch_assoc()) {
                                    ?>
                                        <option value="<?php echo $rowNomSesion['PK'] ?>"><?php echo $rowNomSesion['CITY'] . ' - ' . $rowNomSesion['NOM_SESION'] ?></option>
