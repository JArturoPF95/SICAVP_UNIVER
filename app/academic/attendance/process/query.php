...
//Check in - Check out
$sql_academic_check = "SELECT ASH.PK
    ...
    , ASH.END_CLASS
    , ASH.ACADEMIC_TERM
    , IFNULL( (SELECT DISTINCT ATT.ATTENDANCE_DATE FROM academic_attendance ATT WHERE ( ATT.SCHEDULE_ID = ASH.PK OR (ASH.PERSON_CODE_ID = ATT.ACADEMIC_ID AND ASH.CODE_DAY = ATT.CODE_DAY AND ASH.EVENT_ID = ATT.EVENT_ID AND ASH.PROGRAM = ATT.PROGRAM AND ASH.CURRICULUM = ATT.CURRICULUM AND ATT.ATTENDANCE_DATE BETWEEN ASH.START_DATE AND ASH.END_DATE) ) AND ATT.ATTENDANCE_DATE = DATE(NOW())), '') ATTENDANCE_DATE
    , IFNULL( (SELECT DISTINCT ATT.TINC FROM academic_attendance ATT WHERE ( ATT.SCHEDULE_ID = ASH.PK OR (ASH.PERSON_CODE_ID = ATT.ACADEMIC_ID AND ASH.CODE_DAY = ATT.CODE_DAY AND ASH.EVENT_ID = ATT.EVENT_ID AND ASH.PROGRAM = ATT.PROGRAM AND ASH.CURRICULUM = ATT.CURRICULUM AND ATT.ATTENDANCE_DATE BETWEEN ASH.START_DATE AND ASH.END_DATE) ) AND ATT.ATTENDANCE_DATE = DATE(NOW()) AND ATT.IN_OUT = 1 ORDER BY AttendanceId ASC LIMIT 1), '') TINC
    , IFNULL( (SELECT DISTINCT ATT.ATTENDANCE_TIME FROM academic_attendance ATT WHERE ( ATT.SCHEDULE_ID = ASH.PK OR (ASH.PERSON_CODE_ID = ATT.ACADEMIC_ID AND ASH.CODE_DAY = ATT.CODE_DAY AND ASH.EVENT_ID = ATT.EVENT_ID AND ASH.PROGRAM = ATT.PROGRAM AND ASH.CURRICULUM = ATT.CURRICULUM AND ATT.ATTENDANCE_DATE BETWEEN ASH.START_DATE AND ASH.END_DATE) ) AND ATT.ATTENDANCE_DATE = DATE(NOW()) AND ATT.IN_OUT = 1 ORDER BY AttendanceId ASC LIMIT 1), '') CHECK_IN
    , IFNULL( (SELECT DISTINCT ATT.ATTENDANCE_TIME FROM academic_attendance ATT WHERE ( ATT.SCHEDULE_ID = ASH.PK OR (ASH.PERSON_CODE_ID = ATT.ACADEMIC_ID AND ASH.CODE_DAY = ATT.CODE_DAY AND ASH.EVENT_ID = ATT.EVENT_ID AND ASH.PROGRAM = ATT.PROGRAM AND ASH.CURRICULUM = ATT.CURRICULUM AND ATT.ATTENDANCE_DATE BETWEEN ASH.START_DATE AND ASH.END_DATE) ) AND ATT.ATTENDANCE_DATE = DATE(NOW())  AND ATT.IN_OUT = 2 ORDER BY AttendanceId ASC LIMIT 1), '') CHECK_OUT
    , IFNULL( (SELECT DISTINCT ATT.CLASS_SUMMARY FROM academic_attendance ATT WHERE ( ATT.SCHEDULE_ID = ASH.PK OR (ASH.PERSON_CODE_ID = ATT.ACADEMIC_ID AND ASH.CODE_DAY = ATT.CODE_DAY AND ASH.EVENT_ID = ATT.EVENT_ID AND ASH.PROGRAM = ATT.PROGRAM AND ASH.CURRICULUM = ATT.CURRICULUM AND ATT.ATTENDANCE_DATE BETWEEN ASH.START_DATE AND ASH.END_DATE) ) AND ATT.ATTENDANCE_DATE = DATE(NOW())  AND ATT.IN_OUT = 1  ORDER BY AttendanceId ASC LIMIT 1), '') CLASS_SUMMARY
    , IFNULL( (SELECT DISTINCT ATT.JUSTIFY FROM academic_attendance ATT WHERE ( ATT.SCHEDULE_ID = ASH.PK OR (ASH.PERSON_CODE_ID = ATT.ACADEMIC_ID AND ASH.CODE_DAY = ATT.CODE_DAY AND ASH.EVENT_ID = ATT.EVENT_ID AND ASH.PROGRAM = ATT.PROGRAM AND ASH.CURRICULUM = ATT.CURRICULUM AND ATT.ATTENDANCE_DATE BETWEEN ASH.START_DATE AND ASH.END_DATE) ) AND ATT.ATTENDANCE_DATE = DATE(NOW())  AND ATT.IN_OUT = 1  ORDER BY AttendanceId ASC LIMIT 1), '') JUSTIFY 
FROM academic_schedules ASH 
WHERE ASH.PERSON_CODE_ID = '$user_active' 
    AND DATE(NOW()) BETWEEN ASH.START_DATE AND ASH.END_DATE
...
$sql_get_attendance = "SELECT DISTINCT
    ...
    INNER JOIN academic_schedules ASH ON ASH.PK = ATD.SCHEDULE_ID OR (ASH.PERSON_CODE_ID = ATD.ACADEMIC_ID AND ASH.CODE_DAY = ATD.CODE_DAY AND ASH.EVENT_ID = ATD.EVENT_ID AND ASH.PROGRAM = ATD.PROGRAM AND ASH.CURRICULUM = ATD.CURRICULUM AND ATD.ATTENDANCE_DATE BETWEEN ASH.START_DATE AND ASH.END_DATE)
