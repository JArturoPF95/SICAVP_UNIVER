...
$getSesionPK = "SELECT DISTINCT PK, NOM_SESION FROM code_sesion WHERE CODE_CITY = '$user_city' AND CODE_SESION_NOM = '$user_sesion'";
...
if ($resultSesionPK -> num_rows > 0) {
    while ($rowSesionPK = $resultSesionPK -> fetch_assoc()) {
        ...
        $nomSesion = $rowSesionPK['NOM_SESION'];
    }
}
...
$hojaActiva->setCellValue('C4', 'EDUCACION UNIVERSITARIA UNIVER');
...
$hojaActiva->getColumnDimension('C')->setAutoSize(true);
$hojaActiva->setCellValue('C5', $nomSesion);
...
if ($resultIncidenceReport->num_rows > 0) {
    while ($rows = $resultIncidenceReport->fetch_assoc()) {
        ...
        if ($rows['FLAG_CLINIC'] == '1') { //Prácticas clínicas
            $level = 'LCS';
        } elseif($rows['CURRICULUM'] == 'RLNUTR' or $rows['CURRICULUM'] == 'RLENFE' or $rows['CURRICULUM'] == 'RLPSIC' or $rows['CURRICULUM'] == 'RLFISI') { //Carreras de salud
            $level = 'LCS';
        } elseif($rows['PROGRAM'] == 'BGUNI' or $rows['PROGRAM'] == 'BGUDG' or $rows['PROGRAM'] == 'BGCMAT') {
            $level = 'B';
        } elseif (strpos($rows['GENERAL_ED'], 'OnLine') !== false or strpos($rows['PROGRAM'], 'On') !== false) {
            $level = 'LOL';
        } elseif (strpos($rows['GENERAL_ED'], 'Blend') !== false and (strpos($rows['PROGRAM'], 'Mix') !== false or strpos($rows['PROGRAM'], 'Eje') !== false) and strpos($rows['CURRICULUM'], 'Ref Cu') !== false) {
        ...
        /*if ($rows['FLAG_CLINIC'] == '1') {
            ...
            $tab = '105.00';
        } elseif ( (strpos($rows['PROGRAM'], 'Mae') === false or strpos($rows['PROGRAM'], 'Eje') === false) )  {
            $tab = '200.00';
        }  else {
            $tab = '175.00';
        }*/
        ...
        if ($rows['FLAG_CLINIC'] == '1') { //Prácticas clínicas
            ...
            $tab = '100.46';
        } elseif($rows['PROGRAM'] == 'RMAEJE' or $rows['PROGRAM'] == 'RMAEOL') {
            $tab = '200.00';
        } elseif (strpos($rows['GENERAL_ED'], 'OnLine') !== false or strpos($rows['PROGRAM'], 'On') !== false) {
            $tab = '100.00';
        } elseif (strpos($rows['GENERAL_ED'], 'Blend') !== false and (strpos($rows['PROGRAM'], 'Mix') !== false or strpos($rows['PROGRAM'], 'Eje') !== false) and strpos($rows['CURRICULUM'], 'Ref Cu') !== false) {
        ...
        $hojaActiva->setCellValue('E' . $row, $tab);
        ...
        $incidencesTot = $rows['COUNTSUPS'] - $rows['COUNTTINCS'];

        if ($rows['FLAG_CLINIC'] == '1') { //Prácticas clínicas
            $hojaActiva->setCellValue('O' . $row, $incidencesTot);
        } elseif($rows['CURRICULUM'] == 'RLNUTR' or $rows['CURRICULUM'] == 'RLENFE' or $rows['CURRICULUM'] == 'RLPSIC' or $rows['CURRICULUM'] == 'RLFISI') { //Carreras de salud
            $hojaActiva->setCellValue('N' . $row, $incidencesTot);
        } elseif($rows['PROGRAM'] == 'BGUNI' or $rows['PROGRAM'] == 'BGUDG' or $rows['PROGRAM'] == 'BGCMAT') {
            $hojaActiva->setCellValue('H' . $row, $incidencesTot);
        }  elseif (strpos($rows['GENERAL_ED'], 'OnLine') !== false or strpos($rows['PROGRAM'], 'On') !== false) {
            $hojaActiva->setCellValue('M' . $row, $incidencesTot);
        } elseif (strpos($rows['GENERAL_ED'], 'Blend') !== false and (strpos($rows['PROGRAM'], 'Mix') !== false or strpos($rows['PROGRAM'], 'Eje') !== false) and strpos($rows['CURRICULUM'], 'Ref Cu') !== false) {
            $hojaActiva->setCellValue('L' . $row, $incidencesTot);
        } elseif ((strpos($rows['PROGRAM'], 'Mix') !== false or strpos($rows['PROGRAM'], 'Eje') !== false) and strpos($rows['CURRICULUM'], 'Ref') === false) {
            $hojaActiva->setCellValue('J' . $row, $incidencesTot);
        } elseif (strpos($rows['CURRICULUM'], 'Ref Cu') === false && (strpos($rows['PROGRAM'], 'Mix') === false or strpos($rows['PROGRAM'], 'Eje') === false)) {
            $hojaActiva->setCellValue('I' . $row, $incidencesTot);
        } elseif (strpos($rows['CURRICULUM'], 'Ref Cu') !== false && (strpos($rows['PROGRAM'], 'Mix') === false or strpos($rows['PROGRAM'], 'Eje') === false)) {
            $hojaActiva->setCellValue('K' . $row, $incidencesTot);
        } else {
            $hojaActiva->setCellValue('P' . $row, $incidencesTot);
        }

        $hojaActiva->setCellValue('T' . $row, $incidencesTot);
        $hojaActiva->setCellValue('U' . $row, $incidencesTot);
        $hojaActiva->setCellValue('V' . $row, $incidencesTot);
