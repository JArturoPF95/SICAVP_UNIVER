...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sqlValJustifies = "SELECT ADT.AttendanceId, EMP.ID_NOM, EMP.NAME, EMP.LAST_NAME, EMP.LAST_NAME_PREFIX, ADT.ATTENDANCE_DATE, ADT.ATTENDANCE_TIME, ADT.JUSTIFY, ADT.COMMENTS, CIN.DESCRIP_TINC
            FROM admin_attendance ADT 
            INNER JOIN employed EMP ON EMP.ID_NOM = ADT.NOM_ID 
            INNER JOIN code_incidence CIN ON CIN.CODE_TINC = ADT.TINC
            WHERE JUSTIFY != '' AND (EMP.SUPERVISOR_ID = '$user_active' OR EMP.SUPERVISOR_ID_AUX = '$user_active') AND ADT.ATTENDANCE_DATE BETWEEN (SELECT START_DATE FROM payroll_period WHERE ID = '$payrollPeriodID') AND (SELECT END_DATE FROM payroll_period WHERE ID = '$payrollPeriodID')
            ORDER BY ADT.ATTENDANCE_DATE, EMP.ID_NOM ASC;";
            $resultValJustifies = $mysqli -> query($sqlValJustifies);

} elseif ($_SERVER["REQUEST_METHOD"] == "GET") {
    if (isset($_GET['pe'])) {
        $payrollPeriodID = $_GET['pe'];
        $flag_send = '1';
    } else {
        $flag_send = '0';
        $payrollPeriodID = '0';
...
<body>
    <div class="container-fluid" style="height: 100%; width: 85%;">
        ...
        <div class="row">
            <form class="row gy-2 gx-3 align-items-center" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
            <!--input hidden type="text" name="supervisor" class="form-control" id="autoSizingInput" value="<?php echo $user_active ?>">
            <input hidden type="text" name="flag" class="form-control" id="autoSizingInput" value="1" >
            <div class="col-auto">
                <label class="visually-hidden" for="autoSizingInputGroup">Fecha</label>
                <div class="input-group">
                <div class="input-group-text">Fecha</div>
                <input type="date" class="form-control" name="selectedDate" id="autoSizingInputGroup" value="<?php echo $selectedDate ?>">
                </div>
            </div-->
            <input hidden type="text" name="flag" id="" value="1">
                    <div class="col-sm-5">
                        <select class="form-select" id="specificSizeSelect" name="codePP">
                            ...
                            <?php
                            //$sql_payrollTForm = "SELECT * FROM payroll_period";
            ...
            <div class="col-auto">
                <button type="submit" name="forms" value="payrollSelect" class="btn btn-primary">Consultar</button>
            </div>
            </form>
        </div>
        <div class="row">
            ...
            <table class="table table-primary table-striped table-hover table-bordered my-4 table-sm" style="font-size: 13px;">
                ...
                <?php
                    while($rowJustifies = $resultValJustifies -> fetch_assoc()) {
                        ...
                        $justifyIncidence = $rowJustifies['DESCRIP_TINC'];
                ...
                ?>
                    <tr class="text-center">
                            <td hidden><input type="text" name="attendance[<?php echo $justifyattendance ?>][payrollNom]" id="attendance" value="<?php echo $payrollPeriodID ?>"></td>
                            ...
                            <td hidden><input type="text" name="attendance[<?php echo $justifyattendance ?>][incidence]" id="attendance" value="<?php echo $justifyIncidence ?>"></td>
                            ...
                            <td><?php echo $justifyIncidence ?></td>
                ...
                ?>
                <tr class="text-end">
                        <td colspan="9"><button type="submit" name="forms" value="justifies" class="btn btn-sm btn-primary">Justificar</button></td>
                        </tr>
                </tbody>
