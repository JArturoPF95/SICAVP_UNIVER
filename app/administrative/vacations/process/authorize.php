...
if ($mysqli->query($sql_authorizeDays) === true) {
    if ($authorization == 1) {
        ...
        $sqlValAnti = "SELECT DISTINCT VA.PK, VA.ID_NOM, VA.VACATION_TERM 
                       FROM vacation_anticipated VA
                       INNER JOIN vacation_request VR ON VR.ID_NOM = VA.ID_NOM 
                       WHERE VR.requestId = '$idReq';";
        echo $sqlValAnti;
        $resultValAnti = $mysqli -> query($sqlValAnti);
        if ($resultValAnti -> num_rows > 0) {
            while ($rowValAnti = $resultValAnti -> fetch_assoc()) {
                $idNomUs = $rowValAnti['ID_NOM'];
                $sqlUpdateAnti = "UPDATE vacation_anticipated SET REQUEST_FLAG = '0' WHERE ID_NOM = '$idNomUs';";
